<script lang="ts">
  ...
  let { sites, scanning, onSelect, onScan, onOpen }: {
    sites: Site[]
    scanning: Set<number>
        ...
        <section class="group">
          ...
          <div class="cards">
            {#each group.sites as site}
              {@const active = scanning.has(site.id)}
              <button
                class="card"
                style="--ind: {group.color}; --dim: {group.dimColor}"
              ...
              >
                ...
                <span
                  ...
                  onkeydown={(e) => { if (e.key === 'Enter') { e.stopPropagation(); onOpen(site.url) } }}
                  aria-label="Open {site.name || hostname(site.url)} in browser"
                >↗</span>
                <span
                  role="button"
                  tabindex="0"
                  ...
                  onclick={(e) => { e.stopPropagation(); if (!active) onScan(site.id) }}
                  onkeydown={(e) => { if (e.key === 'Enter' && !active) { e.stopPropagation(); onScan(site.id) } }}
                  aria-label="Scan {site.name || hostname(site.url)}"
                >↻</span>
              </button>
            {/each}
          </div>
...
<style>
  ...
  .card-btn:hover { background: var(--bg-hover); color: var(--text-0); }
  .card-btn[aria-disabled="true"] { opacity: 0.45; cursor: not-allowed; }
